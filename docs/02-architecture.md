...
系统由前端工作台、Spring Boot 后端、PostgreSQL + pgvector、MinIO、OpenAI-compatible 模型服务组成。第一版不引入独立向量数据库和消息队列，使用 PostgreSQL 同时承载业务数据、任务数据和向量数据。
...
sequenceDiagram
  ...
  Worker->>DB: 写入 chunks 和 embedding
  User->>FE: 发起问答
  FE->>API: POST /chat
...
- 后端、前端、数据库、对象存储分离部署。
- 使用外部密钥管理注入敏感环境变量。
- 后续可将 pgvector 替换或扩展为 Milvus/OpenSearch。
