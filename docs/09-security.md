...
- `ADMIN` 是平台管理员，可管理用户角色、租户，并可管理租户内所有知识库。
...
数据表保留 `tenant_id`，知识库相关查询需要校验当前用户访问权限，文档列表、检索和问答都应限定在当前知识库内。当前已支持管理员查看和创建租户，但注册仍默认进入 `Default` 租户；指定租户邀请和跨租户迁移上线前，不应直接在数据库中手工修改用户 `tenant_id`。
...
- 密码复杂度策略。
- 审计日志写入业务操作。
- 更完整的管理员用户管理页面，例如指定租户创建用户、禁用账号、重置密码、审计授权历史。
- 文件病毒扫描和更严格 MIME 校验。
- 前端 XSS 防护策略和 CSP。
